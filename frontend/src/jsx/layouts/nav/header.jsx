...
function Header({ onNote }) {
	...
	const profilePic = user.profilePicture ? (user.profilePicture.startsWith('http') ? user.profilePicture : `http://localhost:5000/${user.profilePicture}`) : IMAGES.user;
	// ... rest of header ...
										...
										<div className="card-body p-0">
											<div className="d-flex profile-media justify-content-between align-items-center">
												<div className="d-flex align-items-center">
													<img src={profilePic} alt="img" width="50" style={{ borderRadius: '50%' }} />
													<div className="ms-3">
														<h4 className="mb-0">{user.name || "User"}</h4>
														<p className="mb-0">{user.email || "demo@mail.com"}</p>
													</div>
												</div>
